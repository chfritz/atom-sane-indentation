...
/** Language specific conditions that, when met by a node whose parent starts
  on the previous row, will cause and indentation. Example:
  const a =
    3 + 5;
...
const previousRowConditions = {
  ...
  python: (node) => (
    ...
    || node.parent.type == 'elif_clause'
    || node.parent.type == 'else_clause'
...
const treeWalk = (node, scopes, previousRowConditions, lastScope = null) => {
  if (node == null || node.parent == null) {
    ...
    (!increment && scopes.indentExceptFirst.get(node.parent.type)
      && node.previousSibling != null && increment++);
...
const suggestedIndentForBufferRow =
  (e, scopes, previousRowConditions = null) => {
