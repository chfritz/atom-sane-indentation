...
getRegex = function(scopes, pattern) {
  var regex = atom.config.get('editor.sane.' + pattern, {
      scope: scopes
    });
  console.log("regex", scopes, pattern, regex);
  if (!regex) {
...
function suggestedIndentForTokenizedLineAtBufferRow(
  //bufferRow, line, tokenizedLine, options) {
  bufferRow, line, tokenizedLine, options, scopes, tabLength) {

    console.log(bufferRow, line, tokenizedLine, options, scopes, tabLength);
    ...
    increaseIndentRegex = getRegex(scopes, 'increaseIndentPattern');
    if (!increaseIndentRegex) {
      // no sane indentation defined: use atom.io's default behavior
      return null;
    }
    decreaseIndentRegex = getRegex(scopes, 'decreaseIndentPattern');
    decreaseNextIndentRegex = getRegex(scopes, 'decreaseNextIndentPattern');
    ...
    desiredIndentLevel = this.indentationForBufferRow(precedingRow);
    if (!increaseIndentRegex) {
    ...
    tokenizedPrecedingLine =
      this.displayBuffer.tokenizedBuffer
    .buildTokenizedLineForRowWithText(precedingRow, precedingLine);

    if (!this.isBufferRowCommented(precedingRow)) {
      if (increaseIndentRegex) {
        desiredIndentLevel +=
      ...
      //         || t == firstToken.value);
      //     })) {
      //   this.temporaryIndentations[bufferRow] = true;
      //   if (!this.temporaryIndentations[precedingRow]) {
      //     desiredIndentLevel += 1;
      //     // console.log("cliff hanger!");
      //   }
      // } else if (this.temporaryIndentations[precedingRow]) {
      ...
      // Comments:
      const increaseRegex = getRegex(scopes, 'comments.increase');
      const decreaseRegex = getRegex(scopes, 'comments.decrease');
      const decreaseNextRegex = getRegex(scopes, 'comments.decreaseNext');
      if (increaseRegex) {
        desiredIndentLevel +=
          countRegex(increaseRegex, precedingLine, tokenizedPrecedingLine);
      }
      if (decreaseNextRegex) {
        desiredIndentLevel -=
          countRegex(decreaseNextRegex, precedingLine, tokenizedPrecedingLine);
      }
      if (decreaseRegex) {
        if (countRegex(decreaseRegex, precedingLine, tokenizedPrecedingLine)) {
          desiredIndentLevel += 1;
        }

        if (countRegex(decreaseRegex, line, tokenizedLine)) {
          desiredIndentLevel -= 1;
        }
      }

      // console.log("commented", desiredIndentLevel, increaseRegex, decreaseRegex, decreaseNextRegex);
    }
  ...
  activate: function(state) {
    ...
    atom.workspace.observeTextEditors(function(e) {
        ...
        e.languageMode.suggestedIndentForBufferRow =
         // () => { console.log("yay!"); return 1; }
         //suggestedIndentForTokenizedLineAtBufferRow(e);
         (bufferRow, tabLength, options) => {
