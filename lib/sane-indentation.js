...
const suggestedIndentForBufferRow = (e, scopesToIndent, scopesToUnindent) => {
  console.log({e, scopesToIndent, scopesToUnindent});
  ...
  return (row, tabLength, options) => {
    ...
    // const indentationEndScopes = filterScopesToIndent(endScopes, syntaxNode);

    log('scopes:', {scopes, indentationScopes, endScopes, syntaxNode} );

    if (endScopes.length < scopes.length) {

      // this doesn't work when there is joint 'luggage' at the end, as in
      // start: ["javascript", "program", "lexical_declaration", "variable_declarator", "parenthesized_expression", "jsx_element", "jsx_element", "jsx_text"]
      // end: ["javascript", "program", "lexical_declaration", "variable_declarator", "parenthesized_expression", "jsx_element", "jsx_text"]
      ...
      // if (indentationEndScopes.length < indentationScopes.length) {
      //   const endingScopes = indentationScopes.slice(indentationEndScopes.length);
      // ^ the problem with this is they are squashed, so "if, statement" is
      // reduced to "if" (on both start and end), and so no end is detected
